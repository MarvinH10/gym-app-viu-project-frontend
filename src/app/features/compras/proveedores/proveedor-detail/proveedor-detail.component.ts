...
export default class ProveedorDetailComponent implements OnInit {
  ...
  readonly detailSections: DetailSection[] = [
    ...
    },
    {
      ...
      fields: [{ name: 'notes', label: 'Observaciones', colSpan: 2, fallback: 'Sin notas' }],
    },
    {
      title: 'Auditoría',
      fields: [
  ...
  ngOnInit() {
    ...
    this.supplierApi.getSupplier(id).subscribe({
      ...
      error: (err) => {
        this.loading.set(false);
        const msg = err?.error?.message || 'No se pudo cargar la información del proveedor.';
        this.error.set(msg);
  ...
  goToEdit() {
    ...
    if (id) this.router.navigate(['/compras/proveedores', id, 'edit']);
  }
