...
import { PurchaseResource, CreatePurchasePayload, PurchaseProductPayload } from '@/core/models/purchase.model';
...
export class CompraCreateEditComponent implements OnInit {
    ...
    private loadFormOptions() {
        // Since purchases/form-options is currently unstable (500) on the backend, 
        // we use fallbacks directly as primary to avoid console errors and alerts.
    ...
    private loadWarehousesFallback() {
        this.warehouseApi.getWarehouses({ per_page: 100 }).subscribe({
            next: (res) => {
                this.warehouses.set(res.data.data.map((w: any) => ({
    ...
    private loadPurchase(id: string) {
        ...
        this.purchaseApi.getPurchase(id).subscribe({
            next: (res) => {
                ...
                if (res.data.lines) {
                    this.productLines.set(res.data.lines.map(p => ({
    ...
    onFormSubmit(data: any) {
        ...
        const request = id ? this.purchaseApi.updatePurchase(id, payload) : this.purchaseApi.createPurchase(payload);
