...
export class ComprasListComponent {
  ...
  readonly selectedPurchase = signal<PurchaseResource | null>(null);
  ...
  readonly actions: TableDetailsAction<PurchaseResource>[] = [
    ...
    },
    {
      ...
      icon: 'banknote',
      show: (p: PurchaseResource) => p.status === 'posted' && p.payment_status === 'unpaid',
      onAction: (p: PurchaseResource) => this.openPaymentDialog(p),
    },
    {
      label: 'Cancelar',
      icon: 'ban',
  ...
  openPaymentDialog(purchase: PurchaseResource) {
    this.alertDialog.confirm({
      zTitle: 'Registrar Pago',
      zContent: `¿Estás seguro de que deseas marcar como pagada la compra <strong>${purchase.serie}-${purchase.correlative}</strong>?`,
      zOkText: 'Confirmar Pago',
      zCancelText: 'Cancelar',
      zOnOk: () => {
        this.purchaseApi.payPurchase(purchase.id).subscribe({
          next: () => {
            toast.success('Pago registrado correctamente');
            this.loadPurchases();
          },
          error: (err) => {
            toast.error(err.error?.message || 'No se pudo registrar el pago');
          },
        });
      },
    });
