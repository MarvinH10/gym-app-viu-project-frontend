...
export class Ventas {
  ...
  readonly actions: TableDetailsAction<SaleResource>[] = [
    ...
    },
    {
      ...
      icon: 'banknote',
      show: (s: SaleResource) => s.status === 'posted' && s.payment_status !== 'paid',
      onAction: (s: SaleResource) => this.confirmPaySale(s),
    },
    {
      label: 'Cancelar',
      icon: 'ban',
  ...
  confirmPaySale(sale: SaleResource) {
    this.alertDialog.confirm({
      zTitle: 'Registrar Pago',
      zContent: `¿Confirmas que se ha recibido el pago de la venta <strong>${sale.serie}-${sale.correlative}</strong> por un total de <strong>S/ ${sale.total}</strong>?`,
      zOkText: 'Sí, registrar pago',
      zCancelText: 'Cancelar',
      zOnOk: () => {
        this.saleApi.paySale(sale.id).subscribe({
          next: () => {
            toast.success('Pago registrado correctamente');
            this.loadSales();
          },
          error: (err) => toast.error(err.error?.message || 'No se pudo registrar el pago'),
        });
      },
    });
  }
