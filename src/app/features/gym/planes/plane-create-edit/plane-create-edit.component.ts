...
export class PlaneCreateEditComponent implements OnInit {
  ...
  readonly visibleFields = computed(() => {
    ...
    return this.fields().filter((f) => {
      if (f.name === 'is_active' && !isEdit) return false;
      return true;
    });
  });
  ...
  onFormSubmit(rawData: any) {
    ...
    const data: any = {
      ...
      time_restricted:
        rawData.time_restricted === true ||
        rawData.time_restricted === 'true' ||
        rawData.time_restricted === 1,
      allows_freezing:
        rawData.allows_freezing === true ||
        rawData.allows_freezing === 'true' ||
        rawData.allows_freezing === 1,
      is_active:
        rawData.is_active !== false && rawData.is_active !== 'false' && rawData.is_active !== 0,
      company_id: parseInt(rawData.company_id, 10),

      duration_months: parseInt(
        rawData.duration_months ||
          Math.floor(parseInt(rawData.duration_days, 10) / 30) ||
          (parseInt(rawData.duration_days, 10) > 0 ? 1 : 0),
        10,
      ),
      max_entries_per_month:
        rawData.max_entries_per_month !== undefined &&
        rawData.max_entries_per_month !== null &&
        rawData.max_entries_per_month !== ''
          ? parseInt(rawData.max_entries_per_month, 10)
          : null,
      product_product_id:
        rawData.product_product_id !== undefined &&
        rawData.product_product_id !== null &&
        rawData.product_product_id !== ''
          ? parseInt(rawData.product_product_id, 10)
          : null,
