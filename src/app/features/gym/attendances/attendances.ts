...
export class AttendancesComponent {
  ...
  readonly columns: TableDetailsColumn<AttendanceResource>[] = [
    ...
    },
    {
      ...
      label: 'Duración',
      type: 'text',
      transform: (v: string, row: AttendanceResource) => {
        if (v) return v;
        if (!row.check_out_time) return 'En curso...';

        const diff = new Date(row.check_out_time).getTime() - new Date(row.check_in_time).getTime();
