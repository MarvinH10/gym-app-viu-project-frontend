...
import { MembershipSubscriptionResource, PaginatedSubscriptionsResponse, SubscriptionQueryParams } from '@/core/models/subscription.model';
    ...
    readonly columns: TableDetailsColumn<MembershipSubscriptionResource>[] = [
        {
            ...
            subTransform: (v: any) => v?.name ? `Plan: ${v.name}` : 'Sin plan',
        },
        {
            key: 'status',
            label: 'Estado',
        ...
        },
        {
            ...
            label: 'Vencimiento',
            type: 'text',
            transform: (v: string) => v ? new Date(v).toLocaleDateString() : '—',
        },
        {
            key: 'days_remaining',
            label: 'Días Rest.',
            type: 'badge',
            badgeVariant: (v: number) => v <= 5 ? 'destructive' : 'outline',
    ...
    readonly tableActions: TableDetailsAction<MembershipSubscriptionResource>[] = [
        {
            label: 'Ver detalles',
            icon: 'eye',
            onAction: (s: MembershipSubscriptionResource) => this.router.navigate(['/gym/subscriptions', s.id]),
        },
        {
            label: 'Editar',
            icon: 'pencil',
            onAction: (s: MembershipSubscriptionResource) => this.router.navigate(['/gym/subscriptions', s.id, 'edit']),
        },
        {
            label: 'Eliminar',
            icon: 'trash',
